...
class Xoo_Aff_Fields{
	...
	public $types = array(), $sections = array(), $settings = array(),  $fields = array(), $setting_options = array();
	public $plugin_slug;
	...
	*/
	...
	public function get_field_html_args( $field_id, $args = array() ){
		...
		$settings_with_options = apply_filters( 'xoo_aff_'.$this->plugin_slug.'_setting_with_options', array(
			'select_list',
			'checkbox_list',
			'checkbox_single',
			'radio', 
			...
			foreach ( $settings_with_options as $setting_key ) {
				if( isset( $settings[ $setting_key ] ) ){
					foreach ( $settings[$setting_key] as $option_key => $option_data ) {
						...
						if( isset($option_data['checked']) && $option_data['checked'] === 'checked' ){
							if( $setting_key === 'checkbox_list' ){
								if( !is_array( $args['value'] ) ){
									$args['value'] = array();
								}
								$args['value'][] = $option_key;
							}
							else{
								$args['value'] = $option_key;
							}
	...
	public function set_defaults(){
		...
		$fields = json_decode( get_option( $this->db_field ), true);

		if( !empty( $fields ) ){
			$this->fields = $this->prepare_fields( $fields );
		}
		else{
			//If no fields are found in the database, use default fields
			$this->fields = $this->prepare_fields( $this->fields );
		}

		update_option( $this->db_field, json_encode( $this->fields ) );
	...
	public function prepare_fields( $fields = array() ){
		...
		foreach ( $fields as $field_id => $field_data ) {
			...
			if( isset( $fields_before[ $field_id ] ) ){
				$fields[ $field_id ]['settings'] = array_merge( $fields_before[ $field_id ]['settings'], $field_data['settings'] ); 
			}
	...
	*/
	...
	private function create_fields_layout_for_js(){
		...
		return apply_filters( 'xoo_el_'.$this->plugin_slug.'_fields_layout', $fields_layout );
	...
	*/
	public function get_input_html( $field_id, $args ){
		...
		$placeholder 		= esc_attr__( $args['placeholder'], $this->plugin_slug );
		...
		if( $args['label'] ){
			$field_html .= '<label for='.$field_id.' class="xoo-aff-label">'.wp_kses_post( __( $args['label'], $this->plugin_slug ) ).'</label>';
		}
		...
		switch ( $input_type ) {
			...
			case 'date':
			case 'number':
				$field_html .= '<input type="' . $input_type . '" class="' . $class . '" name="' . $field_id . '" id="' . $field_id . '" placeholder="' . $placeholder . '"  value="' . $value . '" ' . $custom_attributes . '/>';
				break;


			case 'phone':
				$field_html .= '<input type="text" class="' . $class . '" name="' . $field_id . '" id="' . $field_id . '" placeholder="' . $placeholder . '"  value="' . $value . '" ' . $custom_attributes . '/>';
				break;
			...
			case 'checkbox_single':
				...
				if( !empty( $args['options'] ) ){
					...
					foreach ( $args['options'] as $option_value => $option_label ) {
						...
						$checkbox_html .= '<label>'; 
						$checkbox_html .= '<input type="checkbox" name="'.$field_id.'" class="' . $class .'" value="'.$option_value.'" '.$checked.'>'.wp_kses_post( __( $option_label, $this->plugin_slug ) );
						$checkbox_html .= '</label>';
					}
			...
			case 'radio':
				...
				if( !empty( $args['options'] ) ){
					...
					foreach ( $args['options'] as $option_value => $option_label ) {
						...
						$radio_html .= '<label>'; 
						$radio_html .= '<input type="radio" name="'.$field_id.'" class="' . $class .'" value="'.$option_value.'" '.$checked.'>'.wp_kses_post( __( $option_label, $this->plugin_slug ) );
						$radio_html .= '</label>';
					}
			...
			case 'states':
				...
				$select_html = '<select class="'.$class.'" id="'.$field_id.'" name="'.$field_id.'" '.$custom_attributes.'>';
				...
				foreach ( $args['options'] as $option_value => $option_label ) {
					...
					$select_html .= '<option value="'.$option_value.'" '.$selected.'>'.esc_attr__( $option_label, $this->plugin_slug ).'</option>';
				}
